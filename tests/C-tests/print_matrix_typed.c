    ...
    const int M)
{
        ...
        || matrix_precision == double_complex)
    {
     ...
     const int up = 2; bml_print_bml_matrix(A, 0, up, 0, up);
     ...
     REAL_T * data = calloc(up * up, sizeof(REAL_T)); if (data == NULL)
     {
     fprintf(stderr, "calloc failed!\n"); return -1;}

     FILE * fp2 = fopen(filename, "r"); if (fp2 == NULL)
     {
     fprintf(stderr, "Failed to open %s\n", filename); return -1;}

     //FILE *fp3 = fopen("test_output.dat", "w");

     float realp; float imagp; char sign; for (int i = 0; i < up; i++)
     {
     for (int j = 0; j < up; j++)
     {
...
case single_real:
case double_real:
fscanf(fp2, "%f", &realp); printf("Read: %f\n", realp); data[ROWMAJOR(i, j, up, up)] = realp; break; case single_complex:
case double_complex:
     //read complex number in 3 parts, discarding 'i'
fscanf(fp2, "%f %c %fi", &realp, &sign, &imagp); if (sign == '-') imagp *= -1; REAL_T tmp = realp + imagp * _Complex_I; printf("realp %f\n", REAL_PART(tmp)); printf("imagp %f\n", IMAGINARY_PART(tmp)); data[ROWMAJOR(i, j, up, up)] = tmp; break; default:
     ...
     const double tol = 1.e-3; for (int i = 0; i < up; i++)
     {
     for (int j = 0; j < up; j++)
     {
...
//            if (diff > tol)
     ...
     bml_deallocate(&A); free(data); if (remove(filename) != 0)
     {
     fprintf(stderr, "Failed removing file %s\n", filename); return -1;}
     free(filename); bml_free_memory(A_dense); return 0;}
